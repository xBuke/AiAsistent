...
interface ConversationMessagesParams extends AdminReadParams {
...
interface PatchTicketParams extends AdminReadParams {
  conversationUuid: string;
}
...
function mapStatusToStored(input: string): string {
  const t = input?.trim();
  if (t === 'Otvoreno') return 'open';
  if (t === 'Riješeno') return 'closed';
 ...
 */
async function fetchConversationsWithDetails(
  cityId: string,
  filterNeedsHuman?: boolean
): Promise<any[]> {
  let query = supabase
    .from('conversations')
    .select(`
...
export async function getInboxHandler(
  request: FastifyRequest<{ Params: AdminReadParams }>,
  reply: FastifyReply
) {
...
export async function getConversationsHandler(
  ...
  try {
    ...
    const conversationsWithoutTickets = conversationsWithCounts.filter(
      conv => !ticketConversationIds.has(conv.conversationUuid)
    );
...
export async function getConversationDetailHandler(
  request: FastifyRequest<{ Params: ConversationMessagesParams }>,
  reply: FastifyReply
) {
  // Check authentication
  const session = await getSession(request);
 ...
 */
export async function postConversationNoteHandler(
  request: FastifyRequest<{
    Params: ConversationMessagesParams;
    Body: PostNoteBody;
  }>,
  ...
  const { cityCode, conversationUuid } = request.params;
  const body = (request.body as PostNoteBody) || {};
...
export async function patchConversationHandler(
  ...
  const { cityCode, conversationUuid } = request.params;
  const body = (request.body as PatchConversationBody) || {};
...
export async function getTicketsHandler(
  ...
  try {
    ...
    const intakesByConv = new Map<string, any>();
...
export async function patchTicketHandler(
  ...
  const { cityCode, conversationUuid } = request.params;
  const body = (request.body as PatchTicketBody) || {};
...
export async function getReportsHandler(
  ...
  try {
    ...
    const uniqueConversationIds = new Set(
      (userMessages7d || []).map(m => m.conversation_id)
    );
    ...
    const questionsByDay: Array<{ date: string; count: number }> = [];
    for (let i = 6; i >= 0; i--) {
 ...
 */
async function getConversationTitleHandler(
  request: FastifyRequest<{ Params: ConversationMessagesParams }>,
  reply: FastifyReply
) {
  const session = await getSession(request);
 ...
 */
export async function registerAdminReadRoutes(server: FastifyInstance) {
