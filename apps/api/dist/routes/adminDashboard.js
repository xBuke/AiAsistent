...
function mapFormDefinitionRow(row) {
    return {
        ...
        trigger_doc_slugs: Array.isArray(row.trigger_doc_slugs)
            ? row.trigger_doc_slugs.filter((s) => typeof s === 'string')
            : [],
 ...
 */
export async function getDashboardSummaryHandler(request, reply) {
 ...
 */
export async function getTicketsListHandler(request, reply) {
    const session = await getSession(request);
    if (!session) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
 ...
 */
export async function getTicketDetailHandler(request, reply) {
    const session = await getSession(request);
    if (!session) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
 ...
 */
export async function getKnowledgeGapsListHandler(request, reply) {
    const session = await getSession(request);
    if (!session) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
 ...
 */
export async function getKnowledgeGapDetailHandler(request, reply) {
    const session = await getSession(request);
    if (!session) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
 ...
 */
export async function getQuestionsExamplesHandler(request, reply) {
    const session = await getSession(request);
    if (!session) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
 ...
 */
export async function getAdminFormDefinitionsHandler(request, reply) {
    const session = await getSession(request);
    if (!session) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
    ...
    const { data: rows, error } = await supabase
        ...
        .select('id, city_id, name, slug, description, fields, required_attachments, trigger_doc_slugs, is_active, created_at, updated_at')
 ...
 */
export async function postAdminFormDefinitionsHandler(request, reply) {
    const session = await getSession(request);
    if (!session) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
    ...
    if (!trigger_doc_slugs.every((s) => typeof s === 'string')) {
    ...
    const { data: row, error: insertError } = await supabase
        ...
        .single();
    if (insertError) {
        const isUnique = insertError.code === '23505' ||
 ...
 */
export async function putAdminFormDefinitionsHandler(request, reply) {
    const session = await getSession(request);
    if (!session) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
    ...
    if ('trigger_doc_slugs' in body && body.trigger_doc_slugs !== undefined) {
        ...
        if (!body.trigger_doc_slugs.every((s) => typeof s === 'string')) {
            return reply.status(400).send({ error: 'trigger_doc_slugs must be a string array' });
        }
    ...
    const { data: row, error: updateError } = await supabase
        ...
        .single();
    if (updateError) {
        const isUnique = updateError.code === '23505' ||
 ...
 */
export async function deleteAdminFormDefinitionsHandler(request, reply) {
    const session = await getSession(request);
    if (!session) {
        return reply.status(401).send({ error: 'Unauthorized' });
    }
