...
function hasPersonalIdentifiers(text) {
    // OIB (11 digits)
    if (/\b\d{11}\b/.test(text))
        return true;
    // Phone numbers (Croatian formats: +385, 00385, or local)
    if (/(\+385|00385|\b0\d{1,2}[\s-]?\d{3}[\s-]?\d{3,4}\b)/.test(text))
        return true;
    // Email addresses
    if (/\b[\w.-]+@[\w.-]+\.\w+\b/.test(text))
        return true;
    // Full addresses (contains street name + number pattern, or postal code)
    if (/(ulica|ul\.|street|st\.|adresa|address).*?\d+.*?(\d{5}|\b\d{4,5}\b)/i.test(text))
        return true;
    if (/\b\d{5}\b/.test(text) && /(ulica|ul\.|street|st\.|adresa|address)/i.test(text))
        return true;
    return false;
}
...
app.post('/chat', async (req, res) => {
    ...
    if (supabase) {
        try {
            ...
            if (error)
                console.warn('Supabase insert user message failed:', error.message);
        }
        catch (e) {
            ...
            if (!GROQ_STREAMING) {
                ...
                if (supabase && fullReply) {
                    try {
                        ...
                        if (error)
                            console.warn('Supabase insert assistant message failed:', error.message);
                    }
                    catch (e) {
